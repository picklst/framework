...
class User(
    ...
    isVerified = graphene.Boolean(
        description='Whether the user is a verified celebrity or organization.'
    )
    isProfilePrivate = graphene.Boolean(
        description='Whether the user chosen to keep his profile private. '
                    'A private profile is only viewable to user-approved followers.'
    )
    isBusinessProfile = graphene.Boolean(
        description='Whether the user account represents a business or organization, '
                    'and it has enabled business features'
    )
    bio = graphene.String()
    url = graphene.String()
    ...
    #     from user.models import UserSubscription
    #
    #     if not self.isProfilePrivate or UserSubscription.objects.query(user=self,
